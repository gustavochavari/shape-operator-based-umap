...
def CE(P_sparse, Y, a, b, distance='euclidean'):
    """
    Computa a Entropia Cruzada (CE) entre a matriz de probabilidades no espaço
    de alta dimensionalidade (P) e as coordenadas no espaço de baixa dimensão (Y).
    Considera apenas as entradas não nulas de P.
    """
    Q = prob_low_dim(Y, a, b, distance)
    CE_total = 0.0
    
    P_sparse = P_sparse.tocoo()
    for i, j, P_ij in zip(P_sparse.row, P_sparse.col, P_sparse.data):
        if i == j:
            continue  # ignora a diagonal
        
        q_ij = Q[i, j]
        CE_total += - P_ij * np.log(q_ij + 1e-3) - (1 - P_ij) * np.log(1 - q_ij + 1e-3)

    return CE_total

def CE_gradient(P_sparse, Y, a, b, distance='euclidean'):
    """
    ...
    # Garante que está em formato COO para iteração eficiente
    P_sparse = P_sparse.tocoo()

    for i, j, P_ij in zip(P_sparse.row, P_sparse.col, P_sparse.data):
        if i == j:
            continue  # pula a diagonal
        
        y_diff = Y[i] - Y[j]
        dist_ij = np.linalg.norm(y_diff)

        # Cálculo de q_ij
        q_ij = 1.0 / (1.0 + a * dist_ij**(2 * b))
        factor = 2 * b * (P_ij - q_ij) * a * (dist_ij ** (2 * b - 2))

        grad[i] += factor * y_diff
        grad[j] -= factor * y_diff  # simetria para manter centro de massa estável
...
def umap(dados, target, N_NEIGHBOR):
    ...
    for i in tqdm(range(MAX_ITER), desc="Otimização"):
        y = y - LEARNING_RATE * CE_gradient(P, y, a, b)
        CE_current = CE(P, y, a, b) / 1e+5
...
def curvature_estimation(dados, k):
    ...
    for i in tqdm(range(n), desc="Computing curvatures"):
        ...
        #curvatures[i] = abs(np.linalg.trace(S))/2
...
def k_umap(dados, target, N_NEIGHBOR):
    ...
    for i in tqdm(range(n), desc="Busca binária do Sigma (com curvatura)"):
        ...
        target_k = max(3, int(N_NEIGHBOR-K[i]))
